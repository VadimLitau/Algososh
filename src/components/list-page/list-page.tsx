...
export const ListPage: React.FC = () => {
  ...
  const addInHead = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setValue("");
        setLoaderState(DEF_LOADER_STATE);
      }, TIME);
    }, TIME);
  };
  ...
  const removeByIndex = () => {
    if (index === "0") {
      deleteInHead();
      return;
      // пришлось добавить здесь ретур, иначе выполнялся код ниже и на экране "танцевали" айтемы очереди
    }
    if (Number(index) >= state.length) {
      return;
    } else {
        ...
        <fieldset className={style.fieldset}>
          ...
          <Button
            ...
            disabled={
              ...
              loaderState.addByIndex ||
              state.length - 1 < parseInt(index)
            }
